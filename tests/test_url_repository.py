...
def test_add(mocker, url_repo):
    mocker.patch("page_analyzer.database.PostgresConnection.execute", lambda x, y: None)

    assert url_repo.add_url("https://google.com") is None


def test_get_url(mocker, url_repo, record):
    mocker.patch("page_analyzer.database.PostgresConnection.execute_and_get_item",
                 lambda x, y: record)
